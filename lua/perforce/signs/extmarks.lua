...
local group_base = "perforce_extmark_signs_"
...
function M._new(cfg, hls, name)
	...
	self.ns = vim.api.nvim_create_namespace(self.group)
	return self
end
...
function M:remove(bufnr, start_lnum, end_lnum)
	if start_lnum then
		vim.api.nvim_buf_clear_namespace(bufnr, self.ns, start_lnum - 1, end_lnum or start_lnum)
	else
...
function M:add(bufnr, signs)
	...
	for _, s in ipairs(signs) do
		if not self:contains(bufnr, s.lnum) then
			...
			local ok, err = pcall(vim.api.nvim_buf_set_extmark, bufnr, self.ns, s.lnum - 1, -1, {
				id = s.lnum,
...
function M:contains(bufnr, start, last)
	local marks = vim.api.nvim_buf_get_extmarks(bufnr, self.ns, { start - 1, 0 }, { last or start, 0 }, { limit = 1 })
	return #marks > 0
end
